...
ggplot(per_top5,aes(x= reorder(RequestSource, desc(percent)), y= percent))+
  ...
  geom_text(aes(label= percent),size=4,vjust=-0.7, hjust= 0.5)+
  ylab("Percent (%)")+
  xlab("")+
...
ggplot(top5_service,aes(x= RequestType, y=count))+
  geom_bar(aes(fill= RequestSource), stat = "identity", color= "black")+
  coord_flip()+
  xlab("Service Type")+
  ggtitle("Input Channel Breakdown of Top 5 ServiceTypes")
